...
class TmdbServices {
  ...
  // }
  Future<List<SearchResults>> fetchSearchResults(String query) async {
    try {
      var url =
      ...
      if (response.statusCode == 200) {
        List data = jsonDecode(response.body)['results'];
        var searchResults = data.map((e) => SearchResults.fromMap(e)).toList();
  ...
  Future<PersonDetails> fetchPersonDetails(int id) async {
    try {
      ...
      if (response.statusCode == 200) {
        var data = jsonDecode(response.body);
        var personDetails = PersonDetails.fromMap(data);
  ...
  Future<List<PersonCredits>> fetchPersonCredits(int id) async {
    try {
      var url =
      ...
      if (response.statusCode == 200) {
        List data = jsonDecode(response.body)['cast'];
        var personCredits = data.map((e) => PersonCredits.fromMap(e)).toList();
